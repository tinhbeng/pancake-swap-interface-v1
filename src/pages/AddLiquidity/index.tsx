...
export default function AddLiquidity({
  ...
  const oneCurrencyIsWTOMO = Boolean(
    chainId &&
  ...
  async function onAdd() {
    ...
    if (currencyA === ETHER || currencyB === ETHER) {
      const tokenBIsTOMO = currencyB === ETHER
      estimate = router.estimateGas.addLiquidityETH
      method = router.addLiquidityETH
      args = [
        wrappedCurrency(tokenBIsTOMO ? currencyA : currencyB, chainId)?.address ?? '', // token
        (tokenBIsTOMO ? parsedAmountA : parsedAmountB).raw.toString(), // token desired
        amountsMin[tokenBIsTOMO ? Field.CURRENCY_A : Field.CURRENCY_B].toString(), // token min
        amountsMin[tokenBIsTOMO ? Field.CURRENCY_B : Field.CURRENCY_A].toString(), // eth min
        account,
        deadlineFromNow,
      ]
      value = BigNumber.from((tokenBIsTOMO ? parsedAmountB : parsedAmountA).raw.toString())
    } else {
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${currencyIdA || 'TOMO'}/${newCurrencyIdB}`)
      }
    },
    ...
    <Container>
      ...
      {pair && !noLiquidity && pairState !== PairState.INVALID ? (
        <AutoColumn style={{ minWidth: '20rem', marginTop: '1rem' }}>
          <MinimalPositionCard showUnwrapped={oneCurrencyIsWTOMO} pair={pair} />
        </AutoColumn>
      ) : null}
    </Container>
  )
